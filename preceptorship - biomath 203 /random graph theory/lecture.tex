...
\subsubsection{Closed form formula for pseudoinverse}
...
m_{ik} = \sum_{j=1}^Np_{ij}m_{jk} + 1 \iff \bM^{(n+1)} = \bP\bM^{(n)} + \left(\bone\bone^t - diag(\bone)\right)
\end{align}
If the degree for each node is small, then $\bP$ is a sparse matrix, so the multiplication $\bP\bM^{(n)}$ is linear in the number of non-zero entries. The programming illustrations below uses 50 iterations to estimate $\bM$, which appears to be sufficient for the problems tested.  
